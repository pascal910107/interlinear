...
import { buildSearchIndex } from '../editing/build-search-index';
...
function generatePageClassesCss(
  ...
  const header =
    `/* auto-generated by interlinear/vite — safelist of Tailwind\n` +
...
export function interlinearPlugin(opts: InterlinearPluginOptions): Plugin {
  ...
  return {
    ...
    // Build-only (Vite never calls writeBundle on `serve`). The dev server
    // synthesizes per-doc assets and search from live files via middleware
    // and the /__search endpoint; a static build has neither, so we emit
    // both into the output here: the doc's public/ images and a prebuilt
    // search index the prod reader fetches and searches client-side.
    async writeBundle(outputOptions) {
      const outDir = outputOptions.dir
        ? path.resolve(outputOptions.dir)
        : path.resolve(userCwd, 'dist');
      const docs = await findDocs(docsRoot, pagesDir);
      for (const doc of docs) {
        // 1. Copy the doc's public/ assets (page PNGs, figures) so the pages'
        //    `/<docId>/<file>` URLs resolve on a plain static host.
        const publicDir = path.resolve(doc.dir, 'public');
        if (existsSync(publicDir)) {
          ...
          await Promise.all(
            assets.map(async (rel) => {
              const src = path.resolve(publicDir, rel);
              const dst = path.resolve(outDir, doc.id, rel);
              await mkdir(path.dirname(dst), { recursive: true });
              await copyFile(src, dst);
            }),
          );
        }
        // 2. Emit the per-doc static search index (fetched by SearchBar on
        //    first open in the production reader).
        const index = await buildSearchIndex({
          docId: doc.id,
          docDir: doc.dir,
          pages: doc.pages,
        });
        const indexPath = path.resolve(outDir, doc.id, 'search-index.json');
